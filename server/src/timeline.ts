...
async function getEvents(_req: express.Request, res: express.Response, _next: express.NextFunction) {
  const events = await DB.Instance.getEvents();
  res.send(events);
}

async function postEvent(req: express.Request, res: express.Response, _next: express.NextFunction) {
