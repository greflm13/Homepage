...
_filehost.use(serveStatic(path.join(__dirname, 'filehost/'), { 'index': ['index.html', 'index.htm'] }))
_filehost.use('/node_modules', express.static(path.join(__dirname, '../node_modules')));

async function refresh(req: express.Request, _res: express.Response, next: express.NextFunction) {
