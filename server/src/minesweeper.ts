...
function postLeaderboard(
  ...
  const leaderboard: Leaderboard = {
    minesweeper: { easy: [], medium: [], hard: [], people: [] },
  };
  for (let i = 0; i < 10; i++) {
    if (
      req.body.minesweeper.easy[i] !== null &&
      req.body.minesweeper.easy[i] !== undefined
    ) {
      leaderboard.minesweeper.easy.push(req.body.minesweeper.easy[i]);
    }
    if (
      req.body.minesweeper.medium[i] !== null &&
      req.body.minesweeper.medium[i] !== undefined
    ) {
      leaderboard.minesweeper.medium.push(req.body.minesweeper.medium[i]);
    }
    if (
      req.body.minesweeper.hard[i] !== null &&
      req.body.minesweeper.hard[i] !== undefined
    ) {
  ...
  fs.writeFileSync(
    path.join(__dirname, "./leaderboardMinesweeper.json"),
  ...
  res.send(
    JSON.stringify(
      JSON.parse(
        fs
          .readFileSync(path.join(__dirname, "./leaderboardMinesweeper.json"))
          .toString()
      )
    )
  );
