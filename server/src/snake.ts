...
function postLeaderboard(
  ...
  const leaderboard: Leaderboard = JSON.parse(
    fs.readFileSync(path.join(__dirname, "./leaderboardSnake.json")).toString()
  );
  if (req.body !== null && req.body !== undefined) {
    leaderboard.people.push(req.body);
    leaderboard.people.sort((leftSide, rightSide): number => {
  ...
  fs.writeFileSync(
    path.join(__dirname, "./leaderboardSnake.json"),
  ...
  res.send(
    JSON.stringify(
      JSON.parse(
        fs
          .readFileSync(path.join(__dirname, "./leaderboardSnake.json"))
          .toString()
      )
    )
  );
