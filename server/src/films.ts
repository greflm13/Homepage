...
_films.get("/list/:folder/:subFolder", (req, res, next) =>
  listFolderSubFolder(req, res, next)
);
_films.get("/item/:path", (req, res, next) => getItem(req, res, next));
_films.get("/item/:path/:subpath", (req, res, next) => getItem(req, res, next));
_films.get("/item/:path/:subpath/:subsubpath", (req, res, next) =>
...
function listFolderSubFolder(
  req: express.Request,
  res: express.Response,
  _next: express.NextFunction
) {
  c.list(
    "disk1/films/" + req.params.folder + "/" + req.params.subFolder,
    (err, list) => {
      if (err) {
        if (err.message.includes("No such file or directory")) {
          res.send([{ name: "No such file or directory" }]);
