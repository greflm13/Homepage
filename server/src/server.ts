...
const consolelogger: debugsx.IHandler = debugsx.createConsoleHandler(
  "stdout",
...
export class Server {
  ...
  private constructor() {
    ...
    this._express.use(
      requestLanguage({ languages: ["en-GB", "en-US", "de-DE", "de-AT"] })
    );
    ...
    this._express.use(
      "/de",
      express.static(path.join(__dirname, "./public/de"))
    );
    this._express.use(
      "/en",
      express.static(path.join(__dirname, "./public/en"))
    );
    ...
    this._express.use("/", (req, res, next) =>
      this.languageselector(req, res, next)
    );
    this._express.use(this.error404Handler);
    this._express.use(this.errorHandler);

    // Redirect
    this._appredirect.use((req, res, next) =>
      this.logger(req, res, next, "Redirect")
    );
    this._appredirect.get("*.php", (_req, res, _next) => {
      res.sendFile(path.join(__dirname, "/views/no.html"));
    });
    this._appredirect.get(
      "/err",
      (
        err: any,
        req: express.Request,
  ...
  public start(httpport: number, httpsport?: number): Promise<Server> {
    return new Promise<Server>((_resolve, _reject) => {
      if (httpsport) {
        ...
        const server = https
          .createServer(credentials, this._express)
          .listen(httpsport, () => {
        ...
        const httpserver = http
          .createServer(this._appredirect)
          .listen(httpport, () => {
  ...
  private errorHandler(
    ...
    if (err.toString().startsWith("Error: ENOENT")) {
      ...
      res.status(500).render("error500.pug", {
        ...
        err: err,
        href:
          "mailto:sorogon.developer@gmail.com?subject=Server failed;&body=https://www.sorogon.eu/ failed at " +
          ts +
          " with Error: " +
          err,
