(function() {
  ...
  class AssetGroup {
        ...
        if (cachedResponse !== undefined) {
          ...
          if (this.hashes.has(url)) {
            ...
            if (await this.needToRevalidate(req, cachedResponse)) {
              this.idle.schedule(`revalidate(${this.prefix}, ${this.config.name}): ${req.url}`, async () => {
    ...
    async unhashedResources() {
      ...
      return (await cache.keys()).map(request => request.url).filter(url => !this.hashes.has(url));
    }
    /**
    ...
    async initializeFully(updateFrom) {
      ...
      if (updateFrom !== undefined) {
        ...
        await (await updateFrom.previouslyCachedResources())
          .filter(url => this.config.urls.some(cacheUrl => cacheUrl === url) || this.patterns.some(pattern => pattern.test(url)))
  ...
  class AppVersion {
    ...
    async handleFetch(req, context) {
      ...
      if (isNavigationRequest(req, this.scope.registration.scope, this.adapter) && req.url !== this.manifest.index) {
  ...
  class Driver {
    ...
    async initialize() {
      ...
      try {
        ...
        [manifests, assignments, latest] = await Promise.all([table.read('manifests'), table.read('assignments'), table.read('latest')]);
        ...
        await Promise.all([table.write('manifests', manifests), table.write('assignments', assignments), table.write('latest', latest)]);
      }
    ...
    async deleteAllCaches() {
      await (await this.scope.caches.keys()).filter(key => key.startsWith('ngsw:')).reduce(async (previous, key) => {
